...
import com.kiwi.spring.models.MilestoneBaseController;
import com.kiwi.spring.util.CustomException;
import com.kiwi.spring.util.Encrypter;

@Controller
@RequestMapping(value = "/advisors/clients")
public class MilestoneController extends MilestoneBaseController {
	...
	@ResponseBody
	@RequestMapping(value = "/milestones/sync", method = RequestMethod.POST)
	public String syncMilestone(@RequestParam String EncryptedValue)
			throws Exception {
		...
		MilestoneControllerLog.logger.log(Level.INFO, dateString + "syncMilestone method request: " + decryptedString);
		Log.logger.log(Level.INFO, "MilestoneController...");
		...
		if (decryptedString != null) {
			...
			try {
					...
					if (validateAccessToken((String) jsonObject.get("access_token"))) {
						if (jsonObject.get("advisor_id") != null) {
							...
							if (jsonObject.get("milestones") != null) {
								...
								List<Milestone> wps = getAllDataUsingTimeStamp(
										(String) jsonObject.get("advisor_id"), cl_id, time_stamp);
								...
								if (getArray.size() > 0) {
									for (int i = 0; i < getArray.size(); i++) {
										...
										String milestone_id = (String) objects.get("milestone_id");
										if (milestone_id != null) {

											Milestone m = checkWealthPlanID(milestone_id);
											if (m != null) {

												if (objects.get("title") != null)
													m.setTitle((String) objects
															.get("title"));
												if (objects.get("is_delete") != null)
													m.setIs_delete((Long) objects
															.get("is_delete"));
												m.setUpdated_date(String.valueOf(System
														.currentTimeMillis()));

												getUpdate(m, 1);
												if((Long) objects.get("is_delete") == 1l){
													deleteAllElements((String) objects.get("milestone_id"));
												}
											} else {
				...
				MilestoneControllerLog.logger.log(Level.INFO, dateString + " [Error] : " + e);
			}
	...
	@SuppressWarnings("unchecked")
	@ResponseBody
	@RequestMapping(value = "/shared_milestones/sync", method = RequestMethod.POST)
	public String syncSharedMilestones(@RequestParam String EncryptedValue)
			throws Exception {
		...
		MilestoneControllerLog.logger.log(Level.INFO, dateString + "syncSharedMilstone method request: " + decryptedString);
		Log.logger.log(Level.INFO, "syncSharedMilestonesController...");
				...
				if (jsonObject.get("access_token") != null) {
					if (validateAccessToken((String) jsonObject.get("access_token"))) {
						if (jsonObject.get("trusted_associate_id") != null) {
							...
							List<Milestone> ms = getAllSharedDataUsingTimeStamp(
									(String) jsonObject.get("trusted_associate_id"), time_stamp);

							if (!ms.isEmpty()) {
								for (Milestone p : ms) {
									jsonArray.add(p.getJson(getSharedStatus(p.getWealth_plan_element_id(), (String) jsonObject.get("trusted_associate_id"))));
								}
							}
							...
							responseDetailsJson.put("timestamp",
									System.currentTimeMillis());
							responseDetailsJson.put("milestones", jsonArray);
